...
class KDTree
{
  ...
  private:
     ...
     */
    int _partition(vector<Point<Dim>>& ctor_points, int left, int right, int pivotIndex, int dimension);
    Point<Dim> select(vector<Point<Dim>>& ctor_points, int left, int right, int k, int dimension);

    KDTreeNode * _construct(vector<Point<Dim>>& ctor_points, int begin, int end, int dimension);
    double _distance(const Point<Dim>& point1, const Point<Dim>& point2) const;
    Point<Dim> _findNearestNeighbor(KDTreeNode *croot, const Point<Dim>& query, int dimension) const;
    void _destroyy(KDTreeNode *croot);
    void _destroy();
    KDTree<Dim> _copy(const KDTree<Dim>& rhs);
    Point<Dim> _replace(const Point<Dim>& target, const Point<Dim>& currentBest, const Point<Dim>& potential) const;
};
