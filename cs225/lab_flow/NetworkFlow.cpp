...
const Graph & NetworkFlow::calculateFlow() {
  ...
  while (findAugmentingPath(source_, sink_, path)) {
    vector<Edge> edge_ = residual_.getEdges();
    int cap = pathCapacity(path);
    maxFlow_ += cap;
    for (unsigned i = 0; i < path.size() - 1; i++) {
      if (flow_.edgeExists(path[i + 1], path[i])) {
        flow_.setEdgeWeight(path[i + 1], path[i], residual_.getEdgeWeight(path[i + 1], path[i]) + cap);
      } else {
        flow_.setEdgeWeight(path[i], path[i + 1], residual_.getEdgeWeight(path[i], path[i + 1]) - cap);
      }
