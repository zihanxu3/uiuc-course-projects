...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if (first_larger_idx >= 0 && first_larger_idx < subroot->elements.size() && subroot->elements[first_larger_idx] == key) {
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    if (subroot->is_leaf && find(pair.key) == V()) { 
        size_t ins_idx = insertion_idx(subroot->elements, pair);
        auto itr = subroot->elements.begin() + ins_idx;
